...
class SceneTextDataModule(pl.LightningDataModule):
    ...
    def custom_test_dataloaders(self, data_dir):
        transform = self.get_transform(self.img_size, rotation=self.rotation)
        root = PurePath(self.root_dir, 'test')
        v = LmdbDataset(os.path.join(root, data_dir), self.charset_test, self.max_label_length,
                                self.min_image_dim, self.remove_whitespace, self.normalize_unicode,
                                transform=transform)
        return DataLoader(v, batch_size=self.batch_size, num_workers=self.num_workers,
                              pin_memory=True, collate_fn=self.collate_fn)
